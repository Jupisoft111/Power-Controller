...
namespace Power_Controller
{
    public partial class Ventana_Acerca : Form
    {
        ...
        //internal string Texto_Extra = ((PInvoke.Powrprof.IsPwrHibernateAllowed() ? "Hibernate allowed, " : null) + (PInvoke.Powrprof.IsPwrSuspendAllowed() ? "Suspend allowed, " : null) + (PInvoke.Powrprof.IsPwrShutdownAllowed() ? "Shutdown allowed, " : null)).TrimEnd(", ".ToCharArray());
        ...
        private void Botón_Jupisoft_GitHub_Proyecto_Click(object sender, EventArgs e)
        {
            try
            {
                Program.Ejecutar_Ruta("https://github.com/Jupisoft111/Power-Controller/", ProcessWindowStyle.Normal);
            }
            catch (Exception Excepción) { Depurador.Escribir_Excepción(Excepción != null ? Excepción.ToString() : null); }
        }
